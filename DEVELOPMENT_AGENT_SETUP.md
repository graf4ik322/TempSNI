...
- Нужно создать веб-приложение на чистых технологиях согласно ТЗ в TECHNICAL_SPECIFICATION.md
...
НАЧНИ с изучения документации и создания базовой HTML структуры.
```
...
```
1. Создай базовую HTML структуру index.html
2. Создай структуру папок согласно TECHNICAL_SPECIFICATION.md секция 2.2
3. Настрой основные CSS файлы (main.css, components.css, themes.css)
4. Создай основные JavaScript модули
5. Настрой роутинг через History API
```
...
```
1. Реализуй JavaScript модули для placeholder (текст, изображения, iframe)
2. Создай систему настроек с DOM манипуляциями и localStorage
3. Добавь переключение темы через CSS переменные
4. Реализуй адаптивный дизайн на CSS Grid/Flexbox
5. Создай drag & drop через Drag and Drop API
```
...
```
1. Реализуй работу с iframe для отображения внешних сайтов
2. Добавь валидацию URL и обработку ошибок iframe
3. Создай экспорт/импорт через Blob API и FileReader API
4. Реализуй генерацию URL для шаринга через URL parameters
5. Добавь полноэкранный режим через Fullscreen API
```
...
```
1. Оптимизируй производительность (минификация, сжатие изображений)
2. Добавь Service Worker для кэширования статики
3. Подготовь файлы для статического хостинга
...
❌ НИКОГДА не используй API запросы (fetch, XMLHttpRequest)
❌ НИКОГДА не настраивай CORS прокси
❌ НИКОГДА не используй внешние JavaScript библиотеки
❌ НИКОГДА не полагайся на внешние серверы для данных
✅ ТОЛЬКО iframe для внешнего контента
```
...
✅ СТРОГОЕ соответствие структурам данных из документации
